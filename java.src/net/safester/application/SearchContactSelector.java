...
public class SearchContactSelector extends javax.swing.JFrame {
     ...
     */
    public SearchContactSelector(JFrame caller, Connection theConnection, int origin) {
        //Check param
        if (theConnection == null) {
            throw new IllegalArgumentException("Connection cannot be null");
        }
        if (origin != ORIGIN_FROM && origin != ORIGIN_TO) {
    ...
    private void addEmail() {
        ...
        try {
            ...
            String keyId = (String) jTable1.getValueAt(selectedRow, 2);
            ...
            switch (origin) {
                case ORIGIN_FROM:
                    ((Search) parent).addToSender(nameAndEmailAddress);
                    break;
                case ORIGIN_TO:
                    ((Search) parent).addToRecipient(nameAndEmailAddress);
                    break;
            }
...
//                }
    ...
    private List<PhotoAddressBookLocal> loadAddresses()
        ...
        int userNumber = ((Search)parent).getUserNumber();
        PhotoAddressBookListTransfer addressBookListTransfer = new PhotoAddressBookListTransfer(connection, userNumber);
