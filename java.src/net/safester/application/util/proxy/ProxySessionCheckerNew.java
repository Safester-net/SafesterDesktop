 ...
 *         <br>
 *         check() will return true if an URL maybe download from the server. <br>
 *         <br>
...
public class ProxySessionCheckerNew
{
   ...
   //AwakeFileSession awakeSession = null;
     ...
     * if proxy requires authentication, username/password will be asked in
     * loop.
     * 
    ...
    public boolean check() throws Exception
    {
        ...
        while (true)
        {
            try
            {
                if (! ServerParms.getHOST().contains("localhost"))
                {
                    checkUrlDownload("http://www.google.com");
                }
            ...
            {
                ex.printStackTrace();
                //if (awakeSession.getHttpStatusCode() == 407)
                if (awakeUrl.getHttpStatusCode() == 407)                    
                {
                    ...
                    if (ntlmProxy)
                    {
                        ...
                        httpProxy = new HttpProxy(address,
                                port,
                                dialogProxyAuth.getProxyUsername(),
    ...
    public boolean doDownloadNewVersion() throws Exception
    {
        ...
        //if(! SafeShareItLogin.existsSafeShareItLauncherJar())
        //{
        //    return false;
        //}
        ...
        AwakeFileSession awakeFileSession 
                = new AwakeFileSession(ServerParms.getAwakeSqlServerUrl(), null, null, httpProxy, httpProtocolParameters);
     ...
     * @throws Exception
     *             the Exception thrown by Awake & HttpClient
     */
